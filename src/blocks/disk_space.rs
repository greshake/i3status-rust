...
impl Block for DiskSpace {
    fn update(&mut self) -> Result<Option<Duration>> {
        let statvfs = statvfs(Path::new(self.path.as_str())).block_error("disk_space", "failed to retrieve statvfs")?;
        let mut result;
        ...
        if self.unit == Unit::Percent {
            self.disk_space.set_text(format!("{0} {1:.2}%", self.alias, percentage));
            result = percentage as u64;
        } else if self.show_percentage {
            self.disk_space.set_text(format!("{0} {1} ({2:.2}%) {3:?}", self.alias, converted_str, percentage, self.unit));
        } else {
            self.disk_space.set_text(format!("{0} {1} {2:?}", self.alias, converted_str, self.unit));
        }
