...
pub(crate) fn prepare(config: &Config) -> Result<Arc<BlockPlan>> {
    ...
    BlockPlan::new(vec![
        OutputPlan::new("discharging", format.clone()).icon("icon", IconChoices::one(icons::BAT)),
        OutputPlan::new(
            "charging",
            config.charging_format.with_default_format(&format),
        )
        .icon("icon", IconChoices::one(icons::BAT_CHARGING)),
        OutputPlan::new("full", config.full_format.with_default(" $icon ")?)
            .icon("icon", IconChoices::one(icons::BAT)),
        ...
        OutputPlan::new("missing", config.missing_format.with_default(" $icon ")?)
            .icon("icon", IconChoices::one(icons::BAT_NOT_AVAILABLE)),
    ])
}
