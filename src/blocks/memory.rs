//! This is a simple module for i3status-rust implementing a basic memory usage Widget.
//!
//! By default, the format of this
//!
//!
...
impl Memory {
    ...
    fn format_insert_values(&mut self, mem_state: (usize, usize, usize, usize)) -> String {
        ...
        self.format.render(&self.values)
    }
    ...
    pub fn new(config: Value, tx: Sender<Task>, theme: Value) -> Memory {
        let _memtype: String = get_str_default!(config, "type", "memory");
        Memory {

            name: Uuid::new_v4().simple().to_string(),
            memtype: match _memtype.as_ref() {
                "memory" => Memtype::MEMORY,
                "swap" => Memtype::SWAP,
                _ => panic!(format!("Invalid Memory type: {}", _memtype))
            },
            output: TextWidget::new(theme.clone()).with_text("").with_icon("memory_mem"),
            clickable: match get_str_default!(config, "clickable", "true").as_ref() {
                "true" => true,
                "false" => false,
                _ => panic!("Option clickable accepts only boolean values.")
            },
            format: match config["format"] {
                Value::String(ref e) => {
                    FormatTemplate::from_string(e.clone()).unwrap()
                },
                _ =>FormatTemplate::from_string("abcde{Mp}:{MTg}:{Sp}".to_string()).unwrap()
...
impl Block for Memory
{
    ...
    fn update(&mut self) -> Option<Duration> {
        ...
        // Now, create the string to be shown
            ...
            self.output.set_text(output_text);
//        } else {
//            match self.memtype {
//                Memtype::SWAP => {
//                    self.output.set_icon("memory_swap");
//                    let sp: f32 = 100f32 - 100f32 * (mem_state.3 as f32) / (mem_state.2 as f32);
//                    self.output.set_text(format!(": {:.2}%/{:.2}GiB", sp, mem_state.2 as f32 / (1024f32 * 1024f32)));
//                }
//                Memtype::MEMORY => {
//                    self.output.set_icon("memory_mem");
//                    let mp: f32 = 100f32 - 100f32 * (mem_state.1 as f32) / (mem_state.0 as f32);
//                    self.output.set_text(format!(": {:.2}%/{:.2}GiB", mp, mem_state.0 as f32 / (1024f32 * 1024f32)));
//                }
//            }
//        }
