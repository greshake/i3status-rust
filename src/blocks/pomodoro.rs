...
pub(crate) fn prepare(config: &Config) -> Result<Arc<BlockPlan>> {
    ...
    let base = || IconChoices::one(icons::POMODORO);
    BlockPlan::new(vec![
        OutputPlan::new("idle", format.clone())
            .icon("icon", base())
            .icon("status_icon", IconChoices::one(icons::POMODORO_STOPPED)),
        ...
        OutputPlan::new("running", pomodoro_format.clone())
            .icon("icon", base())
            .icon("status_icon", IconChoices::one(icons::POMODORO_STARTED)),
        OutputPlan::new("paused", pomodoro_format)
            .icon("icon", base())
            .icon("status_icon", IconChoices::one(icons::POMODORO_PAUSED)),
        OutputPlan::new("break", break_format)
            .icon("icon", base())
            .icon("status_icon", IconChoices::one(icons::POMODORO_BREAK)),
    ])
}
