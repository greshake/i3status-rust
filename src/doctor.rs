...
pub fn run(config_arg: &str, font_arg: Option<&str>, skip_live: bool) -> usize {
    ...
    if let Some(check) = &font_check {
        ...
        for (family, resolved) in &check.families {
            ...
            if !family_present(family, resolved, check.has_fc_list) {
...
fn analyze_plan(plan: &crate::block_plan::BlockPlan) -> PlanAnalysis {
    ...
    for output in plan.outputs() {
        ...
        for template in [&output.format().full, &output.format().short] {
...
fn collect_icon_tokens(template: &format_template::FormatTemplate, icon_refs: &mut Vec<String>) {
    for token_list in template.0.iter() {
...
struct FontCheck {
    /// The bar's families joined, for the degraded query used when `fc-list`
    /// is unavailable and per-family coverage cannot be tested.
    pattern: String,
    /// The family that draws plain text: the first configured family that is
    /// actually present.
    base_family: String,
    /// Index of that family in `families`.
...
impl FontCheck {
    fn new(font_arg: Option<&str>) -> Option<Self> {
        ...
        // Also proves fc-match exists; used as the base family only when the
        // bar configured no families of its own.
        let fontconfig_default = fc_match(&pattern, None)?;
        let has_fc_list = std::process::Command::new("fc-list")
            .arg("--version")
            .output()
            .is_ok_and(|out| out.status.success());
        let mut families: Vec<(String, Option<String>)> = families
            .into_iter()
            .map(|family| {
        ...
        if families.is_empty() {
            // No font directive: the bar renders with fontconfig's default,
            // so that is the one family it "configured".
            families.push((fontconfig_default.clone(), Some(fontconfig_default.clone())));
        }
        let base_index = families
            .iter()
            .position(|(family, resolved)| family_present(family, resolved, has_fc_list));
        let base_family = match base_index {
            Some(index) => {
                let (family, resolved) = &families[index];
                // A generic alias is not a family any font carries; report
                // the concrete family it stands for.
                if is_generic_family(family) {
                    resolved
                        .as_deref()
                        .map_or_else(|| family.clone(), first_family)
                } else {
                    family.clone()
                }
            }
            None => fontconfig_default,
        };
        Some(Self {
            pattern,
            base_family,
    ...
    fn classify(&self, c: char) -> GlyphFont {
        ...
        if !self.has_fc_list {
            return self.classify_by_pattern(&charset);
        }
        // The bar falls back per character down its font list, so the first
        // configured family that has the codepoint is the one that draws it.
        // Asking fontconfig to match the whole list at once answers a
        // different question — it ranks by its own rules, and a color emoji
        // font in the list outranks family order for every codepoint, which
        // made every glyph look like it came from that one font.
        for (index, (family, resolved)) in self.families.iter().enumerate() {
            if !family_covers(family, resolved, &charset) {
                continue;
            }
            return if Some(index) == self.base_index {
                GlyphFont::Base
            } else {
                GlyphFont::Configured(family.clone())
            };
        }
        // No configured family has it: whatever fontconfig substitutes is
        // what the user will actually see, and is not what they asked for.
        match fc_match("", Some(&charset)) {
            Some(family) => GlyphFont::Fallback(family),
            None => GlyphFont::Missing,
        }
    }

    /// Without `fc-list` there is no way to ask whether one family covers a
    /// codepoint, so fall back to matching the whole pattern and recognising
    /// the answer. Less accurate — see the note in [`Self::classify`].
    fn classify_by_pattern(&self, charset: &str) -> GlyphFont {
...
/// Whether one configured family has this codepoint, asked of fontconfig:
/// `fc-list` with a family and a charset lists only fonts that satisfy both.
///
/// A generic alias (`monospace`, ...) is not a family any font carries, so it
/// is tested through the concrete family it resolves to. A family that is not
/// installed also resolves to something, but that substitute is not what the
/// user configured, so it is not consulted.
fn family_covers(family: &str, resolved: &Option<String>, charset: &str) -> bool {
    if fc_list_family_provides(family, charset) {
        return true;
    }
    is_generic_family(family)
        && resolved.as_ref().is_some_and(|resolved| {
            resolved
                .split(',')
                .any(|concrete| fc_list_family_provides(concrete, charset))
        })
}

/// Whether a configured family is present at all: generic aliases always
/// resolve to something, and without `fc-list` the best available signal is
/// whether the family resolves to itself.
fn family_present(family: &str, resolved: &Option<String>, has_fc_list: bool) -> bool {
    if is_generic_family(family) {
        return true;
    }
    if has_fc_list {
...
/// Whether any installed font of `family` provides `charset`.
fn fc_list_family_provides(family: &str, charset: &str) -> bool {
    std::process::Command::new("fc-list")
        .arg(format!("{family}:charset={charset}"))
        .arg("family")
        .output()
        .is_ok_and(|out| {
            out.status.success() && !String::from_utf8_lossy(&out.stdout).trim().is_empty()
