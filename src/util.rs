...
static FLAG_RECORDER: std::sync::OnceLock<std::sync::Mutex<Vec<(String, String)>>> =
...
/// (country code, flag) pairs generated since the recorder was enabled.
pub(crate) fn recorded_flags() -> Vec<(String, String)> {
...
fn record_flag(country_code: &str, flag: &str) {
    if let Some(recorder) = FLAG_RECORDER.get()
        && let Ok(mut recorder) = recorder.lock()
    {
        recorder.push((country_code.to_string(), flag.to_string()));
    }
}
