...
Key | Values | Required | Default
----|--------|----------|--------
`device` | `sysfs`: A regex matching devices in `/sys/class/power_supply/` to read from (the first matching device is used).<br />`apcaccess`: IPv4Address/hostname:port<br/>`UPower`: this can be `"DisplayDevice"` or a regex matching any of the other paths found by running `upower --enumerate` (the first matching device is used). | No | `sysfs`: the first battery device found in `/sys/class/power_supply`, with "BATx" or "CMBx" entries taking precedence.<br />`apcaccess`: "localhost:3551"<br />`upower`:  `DisplayDevice``
