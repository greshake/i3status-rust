...
`icons_format` | A string to customise the appearance of each icon. Can be used to edit icons' spacing or specify a font that will be applied only to icons via pango markup. For example, use `" <span font_family='NotoSans Nerd Font'>{icon}</span> "` will set font of the icons to be 'NotoSans Nerd Font' | `" {icon} "`
